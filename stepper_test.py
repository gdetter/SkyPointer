...
while True:
    t1 = threading.Thread(target=motor1.rotate_degrees_by_time, kwargs={'angle':360, 'secs':10})
    t2 = threading.Thread(target=motor2.rotate_degrees_by_time, kwargs={'angle':360, 'secs':10})
    t1.start()
    t2.start()
    t1.join()
    t2.join()
